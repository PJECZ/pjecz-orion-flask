...
def datatable_json():
    ...
    for resultado in registros:
        fecha_termino = resultado.fecha_termino.strftime("%Y-%m-%d") if resultado.fecha_termino != None else "NONE"
        data.append(
            {
                "detalle": {
                    "periodo": resultado.fecha_inicio.strftime("%Y-%m-%d") + " — " + fecha_termino,
...
@personas_nombramientos.route("/personas_nombramientos/nuevo_con_persona/<int:persona_id>", methods=["GET", "POST"])
...
def new_with_persona_id(persona_id):
    ...
    if form.validate_on_submit():
        es_valido = True
        # Validar fecha
        if form.fecha_inicio.data != None and form.fecha_termino.data != None:
        ...
        if es_valido:
            if request.files["archivo"].filename == "":
                ...
                nombramiento = PersonaNombramiento(
                    persona=persona,
                    cargo=safe_string(form.cargo.data, save_enie=True),
                    centro_trabajo=safe_string(form.centro_trabajo.data, save_enie=True),
                    tipo=safe_string(form.tipo.data, save_enie=True),
                ...
                bitacora = Bitacora(
                    ...
                    descripcion=safe_message(f"Nuevo Nombramiento {nombramiento.persona.nombre_completo}"),
                ...
                if es_valido:
                    ...
                    es_exitoso = True
                    try:
                        storage.set_filename(hashed_id=nombramiento.encode_id(), description="NOMBRAMIENTO")
                    ...
                    # Remplazar archivo
                    if es_exitoso:
                        nombramiento.archivo = storage.filename
                        nombramiento.url = storage.url
                        nombramiento.save()
                        # Salida en bitacora
                        bitacora = Bitacora(
                            modulo=Modulo.query.filter_by(nombre=MODULO).first(),
    ...
    return render_template("personas_nombramientos/new_with_persona_id.jinja2", form=form, persona=persona)
    ...
    if form.validate_on_submit():
        ...
        if es_valido:
            if request.files["archivo"].filename == "":
                ...
                nombramiento.cargo = safe_string(form.cargo.data, save_enie=True)
                nombramiento.centro_trabajo = safe_string(form.centro_trabajo.data, save_enie=True)
                nombramiento.tipo = safe_string(form.tipo.data, save_enie=True)
...
@personas_nombramientos.route("/personas_nombramientos/<int:persona_nombramiento_id>/pdf")
def download_pdf(persona_nombramiento_id):
    ...
    descarga_nombre = nombramiento.archivo
...
@personas_nombramientos.route("/personas_nombramientos/ver_archivo_pdf/<int:persona_nombramiento_id>")
def view_file_pdf(persona_nombramiento_id):
    """Ver archivo PDF de PersonaNombramiento para insertarlo en un iframe en el detalle"""
